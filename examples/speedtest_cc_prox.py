...
with odl.util.Timer('Numpy'):
    varlp_cc_prox_factor_npy(abs_f, p, sigma, 10)

with odl.util.Timer('Numpy in-place'):
    varlp_cc_prox_factor_npy(abs_f, p, sigma, 10, out=out)

with odl.util.Timer('Numba CPU'):
    varlp_cc_prox_factor_numba(abs_f, p, sigma, 10, 'cpu')

with odl.util.Timer('Numba parallel'):
    varlp_cc_prox_factor_numba(abs_f, p, sigma, 10, 'parallel')

with odl.util.Timer('Numba CUDA'):
    varlp_cc_prox_factor_numba(abs_f, p, sigma, 10, 'cuda')

with odl.util.Timer('Cython'):
    varlp_cc_prox_factor_cython(abs_f, p, sigma, 10)

with odl.util.Timer('Cython in-place'):
    varlp_cc_prox_factor_cython(abs_f, p, sigma, 10, out=out)

with odl.util.Timer('GpuArray 1st time'):
    varlp_cc_prox_factor_gpuary(abs_f, p, sigma, 10)

with odl.util.Timer('GpuArray 2nd time'):
    varlp_cc_prox_factor_gpuary(abs_f, p, sigma, 10)

with odl.util.Timer('GpuArray in-place'):
    varlp_cc_prox_factor_gpuary(abs_f, p, sigma, 10, out=out_gpu)
